...
template <> struct dimension<Geometry2D::Point> : boost::mpl::int_<2> {};
template <> struct coordinate_system<Geometry2D::Point> {
  typedef cs::cartesian type;
};
template <> struct coordinate_type<Geometry2D::Point> { typedef double type; };
template <> struct tag<Geometry2D::Point> { typedef point_tag type; };
template <> struct access<Geometry2D::Point, 0> {
  static double get(Geometry2D::Point const &p) { return p.x; }
  static void set(Geometry2D::Point &p, double const &value) { p.x = value; }
};
template <> struct access<Geometry2D::Point, 1> {
...
public:
  ...
  getSepareteRandomPoints(const Space &space,
                          const AgentGeometry &agent_geometry,
                          const int number) {
    auto outer_vertice = boost::geometry::exterior_ring(space);
    auto it = boost::begin(outer_vertice);
    ...
    for (int i = 0; i < number; i++) {
      while (true) {
        points[i] = Point(x_distribution(engine), y_distribution(engine));
        if (!checkVertexInside(space, points[i], agent_geometry))
          continue;
        bool separeted = true;
        for (int j = 0; j < i; j++) {
  ...
  static void input_space(Space &space, FILE *input_file) {
    int number_of_holes;
    fscanf(input_file, "%d", &number_of_holes);
